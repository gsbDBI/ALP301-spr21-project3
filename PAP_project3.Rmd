...
Our pre-test questions will gather covariates to get a sense of participant's trust in government and civic engagement. 

Participants will first answer survey questions about confidence in elections for our pre-test dependent variables. Next, we will gather covariates around the participant's level of trust in government at the state and federal levels, and trust in recent presidential elections. After a page break, there will be basic demographic questions for covariates (race, gender, etc.). The participant will then be presented with one of the seven treatment messages. 

After receiving the treatment, participants who did not receive the control treatment will be asked post-test questions on their confidence in state and national elections if RLAs were implemented. This will be followed by additional questions on covariates like political ideology, citizenship, education, civic engagement. These will be useful covariates to analyze our data, gauge political attitudes, and measure the effect of messages for subgroups. By asking these questions after the treatment, we aim to reduce the influence of asking questions about an individual's ideology on the pre-test. This design is a between-subject design, where interventions are implemented at the respondent level.
...
<<<<<<< HEAD
### Treatment **NATE TO SEND**
=======
### Treatment (Nate is going to fill this in)
>>>>>>> 62f1772b2cd4e4688acfa09a66daa0f2357092b9
...
Primary:
We will measure the dependent variables by taking the difference in means between the pre- and post-test dependent variables, measured on a scale of 1-10, to see if there is a noticeable change between the control and treatment messages.
...
Secondary:
We will compare the treatment groups to each other to determine which message is most effective.
...
to RLAs and election integrity: age, education, religious views, and political ideology, and trust in government.
...
We care about the outcome (increasing voter confidence) and understanding which types of people are nudged toward this outcome by particular treatments. Therefore, we plan to examine how a few select treatments interact with particular covariates of interest. 

We believe that bipartisanship, the emotional appeal will have the largest effects compared to the other treatments. 

For low-income and low-educated participants, the message about RLAs with the percentage will have a negative effect. For those that identify as Republicans, the message bad losers will be less effective. Republicans will be less affected overall. 

The covariates of interest party affiliation, belief in government, income and education.
...
*UPDATE*
We hypothesize that all messages will ahve a poisitve effect compared wto the control group except for the message about percentage

are tehre any messages in treatemtne that will outperform?
