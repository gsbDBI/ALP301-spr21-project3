...
df[, c(1,3,4,13:15,16)] <- sapply(df[, c(1,3,4,13:15,16)], as.numeric)
...
df[c(2,5:10,12,17,19:21,23,27,29,30)] <- lapply(df[c(2,5:10,12,17,19:21,23,27,29,30)], factor)
...
df$dv_state_treatement_diff = df$dv_post_state_conf - df$dv_pre_state_conf
...
df$income <- ifelse(df$income == "Less than $10,000", "$0-$10,000", df$income)
df$income <- ifelse(df$income == "Prefer not to say", "", df$income)
df$income <- ifelse(df$income == "$150,000 or more", "$150,000", df$income)

#df <- df[order(df$income),]

#df$income_test <- as.numeric(as.factor(df$income2))
