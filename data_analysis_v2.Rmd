...
# 
df$edu_4college <- ifelse(df$edu == "4-year college degree", 1, 0)
...
reg3 <- lm(dv_post_state_conf ~ dummy_treat + dv_pre_state_conf + age + gender_female + parent_yes + state + factor(income) + hispanic + edu_4college + edu_hs + party, data = df)

# run the regression with controls for national level
reg4 <- lm(dv_post_national_conf ~ dummy_treat + dv_pre_national_conf + age + gender_female + parent_yes + state + factor(income) + hispanic + edu_4college + edu_hs + party, data = df)
...
reg5 <- lm(dv_post_state_conf ~ factor(treatment_group) + dv_pre_state_conf + age + gender_female + parent_yes + state + factor(income) + hispanic + edu_4college + edu_hs + party, data = df)

# national with treatment groups
reg6 <- lm(dv_post_national_conf ~ factor(treatment_group) + dv_pre_national_conf + age + gender_female + parent_yes + state + factor(income) + hispanic + edu_4college, data = df)
```
...
# ages 18-24
# estimatr::difference_in_means(dv_state_treatement_diff ~ dummy_treat, data = df, subset = age < 25)
...
#state level with treatment groups
reg_state_fem <- lm(dv_post_state_conf ~ factor(treatment_group)*factor(gender_female) + dv_pre_state_conf + age + parent_yes + state + factor(income) + hispanic + edu_4college + edu_hs + party, data = df)

# national with treatment groups
reg_natl_fem <- lm(dv_post_national_conf ~ factor(treatment_group)*factor(gender_female) + dv_pre_national_conf + age + parent_yes + state + factor(income) + hispanic + edu_4college, data = df)
