...
balance_lm <- lm(as.numeric(treatment_group) ~ birthyear + gender + parent + state + trust_federal + trust_state + accuracy_2016 + accuracy_2020 + income + race + edu + libcon + party, data = df) # factor(region) encodes the string variable as a factor for analysis
...
car::linearHypothesis(balance_lm, c("age = 0", "persons = 0", "newreg = 0",
                                    "vote00 = 0", "vote98 = 0",
                                    "female = 0"),
...
# using the filtered dataset, check how many are assigned to each treatment group in each region
with(df, table(treatment_group, state)) %>% 
  knitr::kable() %>%   # table generator
  add_header_above(c('treatment', 'state'=50)) %>%
