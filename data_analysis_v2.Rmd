...
no_na_df <- df %>% drop_na('age', 'state', 'income', 'party', 'gender_female', 'parent_yes', 'hispanic', 'edu_4college', 'edu_hs')

love.plot(treatment_group_num ~ age + gender_female + parent_yes + state + income + hispanic + edu_4college + edu_hs + dv_pre_state_conf + dv_pre_national_conf + party,
                 data = no_na_df, 
          binary = "std", 
          limits = c(-.5, .5),
          thresholds = c(m = .1)) # threshold determines the dotted lines on the graph

m.out <- matchit(dummy_treat ~ age + gender_female + parent_yes + state + income + hispanic + edu_4college + edu_hs + dv_pre_state_conf + dv_pre_national_conf + party,
                 data = no_na_df, 
...
balance_lm <- lm(treatment_group_num ~ age + gender_female + parent_yes + state + income + hispanic + edu_4college + edu_hs + party + dv_pre_state_conf + dv_pre_national_conf, data = no_na_df) # factor(region) encodes the string variable as a factor for analysis
...
car::linearHypothesis(balance_lm, c("gender_female = 0", "age = 0", "parent_yes = 0", "hispanic = 0", "edu_hs = 0", "dv_pre_state_conf = 0", "dv_pre_national_conf = 0"),
...
reg1 <- lm_robust(dv_post_state_conf ~ dummy_treat + dv_pre_state_conf, data = df)

# run the regression for national confidence level
reg2 <- lm_robust(dv_post_national_conf ~ dummy_treat + dv_pre_national_conf, data = df)

# run the regression with controls for state level
reg3 <- lm_robust(dv_post_state_conf ~ dummy_treat + dv_pre_state_conf + age + gender_female + parent_yes + state + factor(income) + hispanic + edu_4college + edu_hs + party, data = df)

# run the regression with controls for national level
reg4 <- lm_robust(dv_post_national_conf ~ dummy_treat + dv_pre_national_conf + age + gender_female + parent_yes + state + factor(income) + hispanic + edu_4college + edu_hs + party, data = df)
...
reg5 <- lm(dv_post_state_conf ~ treatment_group + dv_pre_state_conf + age + gender_female + parent_yes + state + factor(income) + hispanic + edu_4college + edu_hs + party, data = df)

# national with treatment groups
reg6 <- lm(dv_post_national_conf ~ treatment_group + dv_pre_national_conf + age + gender_female + parent_yes + state + factor(income) + hispanic + edu_4college, data = df)
...
```{r lm_robust}
# Use the lm_robust function from the estimatr package for robust s.e.
estimatr::lm_robust(dv_post_state_conf ~ dummy_treat, data = df)
```
...
reg_state_fem <- lm(dv_post_state_conf ~ treatment_group:gender_female + dv_pre_state_conf + age + parent_yes + state + factor(income_clean) + hispanic + edu_4college + edu_hs + party, data = df)
...
reg_natl_fem <- lm(dv_post_national_conf ~ treatment_group:gender_female + dv_pre_national_conf + age + parent_yes + state + factor(income_clean) + hispanic + edu_4college, data = df)
...
#state level 
reg_state_age <- lm(dv_post_state_conf ~ treatment_group:age + gender_female + dv_pre_state_conf + parent_yes + state + factor(income_clean) + hispanic + edu_4college + edu_hs + party, data = df)

plot_model(reg_state_age, type = "int", terms = c("treatment_group", "age"))

plot_model(reg_state_age, type = "pred", terms = c("treatment_group", "age"))

# national level 
reg_natl_age <- lm(dv_post_national_conf ~ treatment_group:gender_female + dv_pre_national_conf + age + parent_yes + state + factor(income) + hispanic + edu_4college, data = df)
...
```{r educ interaction}
# education
#df$edu_4college <- to_factor(df$edu_4college)

# state only HS 
reg_state_edu_hs <- lm(dv_post_state_conf ~ treatment_group:edu_hs + edu_4college + age + gender_female + dv_pre_state_conf + parent_yes + state + factor(income_clean) + hispanic  + party, data = df)
...
# national only HS
reg_natl_edu_hs <- lm(dv_post_national_conf ~ treatment_group:edu_hs + edu_4college + age + gender_female + dv_pre_national_conf + parent_yes + state + factor(income_clean) + hispanic  + party, data = df)
...
reg_state_edu <- lm(dv_post_state_conf ~ treatment_group:edu_4college + age + gender_female + dv_pre_state_conf + parent_yes + state + factor(income_clean) + hispanic + edu_hs + party, data = df)
...
reg_natl_edu <- lm(dv_post_national_conf ~ treatment_group:edu_4college + age + gender_female + dv_pre_national_conf + parent_yes + state + factor(income_clean) + hispanic + edu_hs + party, data = df)
...
```{r income}
#df$income_clean <- to_factor(df$income_clean)

# income - figure out better way to visualize

# state 
reg_state_inc <- lm(dv_post_state_conf ~ treatment_group:income_clean + edu_hs + edu_4college + age + gender_female + dv_pre_state_conf + parent_yes + state + hispanic  + party, data = df)
...
# national
reg_natl_inc <- lm(dv_post_national_conf ~ treatment_group:income_clean + edu_hs + edu_4college + age + gender_female + dv_pre_national_conf + parent_yes + state + hispanic  + party, data = df)
...
reg_state_never <- lm(dv_post_state_conf ~ treatment_group:never_believer + income_clean + edu_hs + edu_4college + age + gender_female + dv_pre_state_conf + parent_yes + state + hispanic  + party, data = df)
...
reg_natl_never <- lm(dv_post_national_conf ~ treatment_group:never_believer + income_clean + edu_hs + edu_4college + age + gender_female + dv_pre_national_conf + parent_yes + state + hispanic  + party, data = df)
...
reg_state_always <- lm(dv_post_state_conf ~ treatment_group:always_believer + income_clean + edu_hs + edu_4college + age + gender_female + dv_pre_state_conf + parent_yes + state + hispanic  + party, data = df)
...
reg_natl_always <- lm(dv_post_national_conf ~ treatment_group:always_believer + income_clean + edu_hs + edu_4college + age + gender_female + dv_pre_national_conf + parent_yes + state + hispanic  + party, data = df)
...
reg_state_party <- lm(dv_post_state_conf ~ treatment_group:party + income_clean + edu_hs + edu_4college + age + gender_female + dv_pre_state_conf + parent_yes + state + hispanic, data = df)
...
# national level
reg_natl_party <- lm(dv_post_national_conf ~ treatment_group:party + income_clean + edu_hs + edu_4college + age + gender_female + dv_pre_national_conf + parent_yes + state + hispanic, data = df)
...
# race (hispanic) do we want to look at other races?

# state level hispanic Y/N
#df$hispanic <- to_factor(df$hispanic)

reg_state_race <- lm(dv_post_state_conf ~ treatment_group:hispanic + income_clean + edu_hs + edu_4college + age + gender_female + dv_pre_state_conf + parent_yes + state  + party, data = df)

plot_model(reg_state_race, type = "int", terms = c("treatment_group", "hispanic"))
plot_model(reg_state_race, type = "pred", terms = c("hispanic", "treatment_group"))

# national level hispanic Y/N
reg_natl_race <- lm(dv_post_national_conf ~ treatment_group:hispanic + party + income_clean + edu_hs + edu_4college + age + gender_female + dv_pre_national_conf + parent_yes + state + hispanic, data = df)

plot_model(reg_natl_race, type = "int", terms = c("treatment_group", "hispanic"))
plot_model(reg_natl_race, type = "pred", terms = c("hispanic", "treatment_group"))


```

```{r filter out extreme views pre test}
reg_middle_views <- lm(dv_post_state_conf ~ treatment_group + hispanic + income_clean + edu_hs + edu_4college + age + gender_female + dv_pre_state_conf + parent_yes + state  + party, data = df, subset = (dv_pre_state_conf < 10 & dv_pre_state_conf > 1))

plot_model(reg_middle_views, type = "pred", terms = c("treatment_group"))
