...
reg3_att <- lm(dv_post_state_conf ~ dummy_treat + dv_pre_state_conf + age + gender_female + parent_yes + state + income_clean + race_hispanic + race_white + race_black + race_asian + edu_4college + edu_hs + party, data = atten_check_df)

reg4_att <- lm(dv_post_national_conf ~ dummy_treat + dv_pre_national_conf + age + gender_female + parent_yes + state + income_clean + race_hispanic + race_white + race_black + race_asian + edu_4college + edu_hs + party, data = atten_check_df)
...
# run the regression with controls for state level
reg3_att_con <- lm(dv_post_state_conf ~ dummy_treat + dv_pre_state_conf + age + gender_female + parent_yes + state + income_clean + race_hispanic + race_white + race_black + race_asian + edu_4college + edu_hs + party + attention_pass, data = df)

# results look the same for state level w/ and w/out attention pass

# run the regression with controls for national level
reg4_att_con <- lm(dv_post_national_conf ~ dummy_treat + dv_pre_national_conf + age + gender_female + parent_yes + state + income_clean + race_hispanic + race_white + race_black + race_asian + edu_4college + edu_hs + party + attention_pass, data = df)
...
reg5_att <- lm(dv_post_state_conf ~ treatment_group + dv_pre_state_conf + age + gender_female + parent_yes + state + income_clean + race_hispanic + race_white + race_black + race_asian + edu_4college + edu_hs + party, data = atten_check_df)

# results are different for those did and did not fail the attention check at state level

reg6_att <- lm(dv_post_national_conf ~ treatment_group + dv_pre_national_conf + age + gender_female + parent_yes + state + income_clean + race_hispanic + race_white + race_black + race_asian + edu_4college + edu_hs + party, data = df)
...
#state level with treatment groups
reg5_att_con <- lm(dv_post_state_conf ~ treatment_group + dv_pre_state_conf + age + gender_female + parent_yes + state + income_clean + race_hispanic + race_white + race_black + race_asian + edu_4college + edu_hs + party + attention_pass, data = df)

# looks the same at the state level

# national with treatment groups
reg6_att_con <- lm(dv_post_national_conf ~ treatment_group + dv_pre_national_conf + age + gender_female + parent_yes + state + income_clean + race_hispanic + race_white + race_black + race_asian + edu_4college + edu_hs + party + attention_pass, data = df)
...
stargazer(reg5, reg6,
          ...
          column.labels=c("state treatment with controls", "national treatment with controls"),
          title = "Basic Regression Results for Treatment Effect")
```
...
# state 
reg_state_inc <- lm(dv_post_state_conf ~ treatment_group*income_num +  dv_pre_state_conf + age + gender_female + parent_yes + state + race_hispanic + race_white + race_black + race_asian + edu_4college + edu_hs + party, data = df)

summary(reg_state_inc)

plot_model(reg_state_inc, type = "int", terms = c("treatment_group", "income_num"))
plot_model(reg_state_inc, type = "pred", terms = c("income_num", "treatment_group"))

# national
reg_natl_inc <- lm(dv_post_national_conf ~ treatment_group*income_num + dv_pre_national_conf + age + gender_female + parent_yes + state + race_hispanic + race_white + race_black + race_asian + edu_4college + edu_hs + party, data = df)

summary(reg_natl_inc)

plot_model(reg_natl_inc, type = "int", terms = c("treatment_group", "income_num"))
plot_model(reg_natl_inc, type = "pred", terms = c("income_num", "treatment_group"))
```

```{r regression table interaction terms, echo=FALSE, message=TRUE, warning=FALSE, results='asis'}
# regression table for interaction terms
stargazer(reg_state_fem, reg_natl_fem,
          omit = c("income_clean", "dv_pre_state_conf", "dv_pre_national_conf","age", "parent_yes", "state", "race_hispanic", "race_white", "race_black", "race_asian", "edu_4college", "edu_hs", "party"),
          column.labels=c("state gender interaction", "national gender interaction")
          )
        
stargazer(reg_state_age, reg_natl_age,
          omit = c("income_clean", "dv_pre_state_conf", "dv_pre_national_conf","gender_female", "parent_yes", "state", "race_hispanic", "race_white", "race_black", "race_asian", "edu_4college", "edu_hs", "party"),
          column.labels=c("state age interaction", "national age interaction")
          )

stargazer(reg_state_edu_hs, reg_natl_edu_hs, reg_state_edu, reg_natl_edu,         
          omit = c(
            "income_clean",
            "dv_pre_state_conf",
            "dv_pre_national_conf",
            "gender_female",
            "parent_yes",
            "state",
            "race_hispanic",
          ...
          column.labels = c(
            "state education HS interaction",
            "national education HS interaction",
            "state college interaction",
            "national college interaction",
            single.row = TRUE
          )
)

stargazer(reg_state_inc, reg_natl_inc,
          omit = c("dv_pre_state_conf", "dv_pre_national_conf","gender_female", "parent_yes", "state", "race_hispanic", "race_white", "race_black", "race_asian", "age", "party", "edu_hs", "edu_4college"),
          column.labels=c("state income interaction", "national income interaction"))
...
reg_state_hisp <- lm(dv_post_state_conf ~ treatment_group*race_hispanic + dv_pre_state_conf + age + gender_female + parent_yes + state + income_clean +  + race_white + race_black + race_asian + edu_4college + edu_hs + party, data = df)

#summary(reg_state_race)

plot_model(reg_state_hisp, type = "int", terms = c("treatment_group", "race_hispanic"))
plot_model(reg_state_hisp, type = "pred", terms = c("race_hispanic", "treatment_group"))

# national level race_hispanic Y/N
reg_natl_hisp <- lm(dv_post_national_conf ~ treatment_group*race_hispanic + dv_pre_national_conf + age + gender_female + parent_yes + state + income_clean +  + race_white + race_black + race_asian + edu_4college + edu_hs + party, data = df)

#summary(reg_natl_race)

plot_model(reg_natl_hisp, type = "int", terms = c("treatment_group", "race_hispanic"))
plot_model(reg_natl_hisp, type = "pred", terms = c("race_hispanic", "treatment_group"))
